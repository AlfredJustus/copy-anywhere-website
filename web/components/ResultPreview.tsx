...
import { CheckIcon, WarnIcon, XIcon } from "@/components/ui/icons";
...
export function ResultPreview({ blocks, formatSlug, onReset, sourceHtml, inputSource }: ResultPreviewProps) {
  ...
  return (
    <div className="animate-slideUp rounded-2xl overflow-clip ring-1 ring-border shadow-lg scroll-mt-16" ref={containerRef}>
                ...
                >
                  {copiedFormat === key ? (
                    ...
                    <>
                      ...
                      <span className="hidden sm:inline">{FORMATS[key].label === "Markdown" ? "Obsidian" : FORMATS[key].label}</span>
                    </>
                  )}
                </Button>
              ))}
      ...
      <div className="flex flex-col sm:flex-row items-center sm:justify-between px-5">
        <FeedbackWidget blocks={blocks} sourceHtml={sourceHtml} inputSource={inputSource} />
        {!showingPdf && (
          <button
            onClick={handleShare}
            disabled={shareState === "sharing"}
            className="flex items-center gap-1.5 px-2 py-1.5 mb-3 sm:mb-0 rounded-md text-xs text-muted-foreground hover:text-foreground hover:bg-muted transition-colors disabled:opacity-50"
          >
            {shareState === "shared" ? (
              <>
                <CheckIcon />
